...
namespace AionDPS
{
    ...
    class LogRegex : SingletonBase<LogRegex>
    {
        ...
        public static Analyzed getLogResult(string log, string hittedObjectName = @"[가-힣0-9A-Za-z\s]+")
        {
            ...
            Regex rx4 = new Regex(loggedTimestamp + @"(?<isCritical>치명타! )?((?<userName>[가-힣A-Za-z\s]+)(가|이) )?((심연의 폭풍+ (I)?(I)?(V)?(I)?(I)?)을 사용해 )" + $@"(?<hittedObjectName>{hittedObjectName})" + @"(에게|이) (?<damage>[0-9,]+)의 (치명적인 )?대미지를 (받고|받았|줬습|주고)");
